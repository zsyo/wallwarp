...
use crate::ui::style::{
    BUTTON_COLOR_GRAY, BUTTON_COLOR_GREEN, BUTTON_COLOR_RED, BUTTON_COLOR_YELLOW,
};
        ...
        DownloadStatus::Downloading => {
            ...
            let progress_bar = container(
                progress_bar(0.0..=1.0, task.progress)
                    .style(crate::ui::common::styled_progress_bar(theme_colors)),
            )
            .width(Length::Fixed(160.0))
            .height(Length::Fixed(10.0));
            let progress_text = text(format!("{:.0}%", task.progress * 100.0))
                .size(crate::ui::style::TABLE_TEXT_SIZE)
