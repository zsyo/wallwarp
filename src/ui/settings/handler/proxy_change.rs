...
impl App {
    pub(in crate::ui::settings) fn settings_proxy_protocol_changed(
        &mut self,
        protocol: String,
    ) -> Task<AppMessage> {
    ...
    pub(in crate::ui::settings) fn settings_proxy_address_changed(
        &mut self,
        address: String,
    ) -> Task<AppMessage> {
    ...
    pub(in crate::ui::settings) fn settings_proxy_port_changed(
        &mut self,
        port: u32,
    ) -> Task<AppMessage> {
    ...
    pub(in crate::ui::settings) fn settings_proxy_toggled(
        &mut self,
        enabled: bool,
    ) -> Task<AppMessage> {
    ...
    pub(in crate::ui::settings) fn settings_save_proxy(&mut self) -> Task<AppMessage> {
        ...
        let is_port_valid =
            self.settings_state.proxy_port >= 1 && self.settings_state.proxy_port <= 65535;
        ...
        if self.settings_state.proxy_enabled {
            ...
            if is_address_valid && is_port_valid {
                ...
                self.show_notification(
                    self.i18n.t("settings.proxy-save-failed"),
            ...
            let old_proxy = self.config.global.proxy.clone();
            info!(
                "[设置] [代理] 保存（已禁用）: proxy_enabled=false, proxy={}",
                old_proxy
            );
