...
impl App {
    ...
    pub(in crate::ui::settings) fn settings_save_proxy(&mut self) -> Task<AppMessage> {
        ...
        if is_address_valid && is_port_valid {
            // 地址和端口都有效，保存代理设置
            let proxy_url = format!("{}://{}:{}", self.settings_state.proxy_protocol, self.settings_state.proxy_address, self.settings_state.proxy_port);
            ...
            self.settings_state.proxy_address = String::new();
