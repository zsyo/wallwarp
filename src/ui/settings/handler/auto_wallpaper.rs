...
impl App {
    ...
    pub(in crate::ui::settings) fn settings_auto_change_interval_selected(
        ...
        // 根据选择的间隔启动或停止定时任务
        if matches!(self.auto_change_interval, WallpaperAutoChangeInterval::Off) {
            // 选择关闭，停止定时任务
    ...
    pub(in crate::ui::settings) fn settings_custom_interval_minutes_changed(
        ...
        if matches!(self.auto_change_interval, WallpaperAutoChangeInterval::Custom(_)) {
            // 同时更新 UI 状态和配置文件
            self.auto_change_interval = WallpaperAutoChangeInterval::Custom(minutes);
